...
from datman.kv_store import save_kv, load_kv
...
class CachedDataset:
    def __init__(self,
                 ...
                 backend: IOBackend = NumpyBackend()) -> None:
    ...
    def _build_cache(self) -> None:
        for idx in range(len(self.dataset)):
            data_path = self.root / f"data_{idx:05d}{self.backend.extension()}"
    ...
    def __getitem__(self, idx: int) -> dict:
        if self.keep_in_memory and idx in self.cache:
...
class SimpleCache():
    def __init__(self,
                 root: Union[str, Path],
                 backend: IOBackend = NumpyBackend(),
                 keep_in_memory: bool = True):
